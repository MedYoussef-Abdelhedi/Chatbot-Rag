...
# --- CHANGEMENT 1 : MODÈLE PLUS PUISSANT ---
model_name = 'sentence-transformers/paraphrase-multilingual-mpnet-base-v2'

# --- CHANGEMENT 2 : DIMENSION AUGMENTÉE (384 -> 768) ---
MODEL_DIMENSION = 768 

print(f"Chargement du nouveau modèle : {model_name} (Dim: {MODEL_DIMENSION})...")
...
def read_file_content(file_path: str) -> list[str]:
    ...
    if text:
        ...
        # On garde les phrases un peu plus longues pour ce modèle (minimum 3 caractères)
        filtered_list = [line.strip() for line in text_list if len(line.strip()) > 3 and not line.strip().startswith("<")]
...
def similar_corpus(input_corpus: str, model: SentenceTransformer, db_conn_str: str, limit: int = 5) -> list[tuple]:
    ...
    with psycopg.connect(db_conn_str) as conn:
        with conn.cursor() as cur:
            # Notez le ::vector qui est essentiel
            cur.execute("""
...
def main_insertion_all_files():
    try:
        with psycopg.connect(db_connection_str) as conn:
            ...
            with conn.cursor() as cur:
                print("\n--- DÉBUT DE LA MIGRATION VERS LE NOUVEAU MODÈLE ---")
                
                print(f"1. Suppression de l'ancienne table (Dim 384)...")
                cur.execute("CREATE EXTENSION IF NOT EXISTS vector")
                cur.execute("DROP TABLE IF EXISTS embeddings")

                print(f"2. Création de la nouvelle table (Dim {MODEL_DIMENSION})...")
                cur.execute(f"""
                ...
                if not os.path.exists(data_folder_path):
                    print(f"ERREUR : Dossier '{data_folder_path}' introuvable.")
                    return

                all_files = [f for f in os.listdir(data_folder_path) if f.endswith(".txt")]
                print(f"3. {len(all_files)} fichiers trouvés.")
                ...
                print(f"\n--- SUCCÈS : {total_phrases} phrases insérées avec le modèle 768 dimensions ! ---")
...
if __name__ == "__main__":
    # 1. On réinitialise tout avec le nouveau modèle
    main_insertion_all_files() 

    # 2. Test
    query = "Quelle est la procédure pour une demande de congé ?"
    print(f"\n--- Recherche (Modèle 768 dim) pour : '{query}' ---")
